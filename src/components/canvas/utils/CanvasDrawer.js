...
export default class CanvasDrawer {
  ...
  drawAngleLine(radians) {
    ...
    if (this.trigVisible.cos) {
      ...
      this.ctx.fillText(
        Math.cos(radians).toFixed(2),
        lineEndX + (circleCentreX - lineEndX) / 2,
        lineEndY
      );
    }
    ...
    if (this.trigVisible.sin) {
      ...
      this.ctx.fillText(
        Math.sin(radians).toFixed(2),
        lineEndX,
        lineEndY + (circleCentreY - lineEndY) / 2
      );
    }
  ...
  resetCanvas() {
    ...
    if (this.circleDetails.degrees) {
      ...
      for (let i = 0; i < 360; i += 1) {
        ...
        const [lineStartX, lineStartY] = this.circle.circleEndOffsetCoords(
          radiants,
          -degreeUnitLen
        );
    ...
    if (this.circleDetails.radians) {
      ...
      for (let i = 0.0; i < 2 * Math.PI; i += 0.1) {
        ...
        const [lineEndX, lineEndY] = this.circle.circleEndOffsetCoords(
          i,
          radianUnitLen
        );
    ...
    if (this.circleDetails.degrees) {
      ...
      for (let i = 0; i < 360; i += 10) {
        ...
        const [posX, posY] = this.circle.circleEndOffsetCoords(
          radians,
          -degreeUnitValueLen
        );
    ...
    if (this.circleDetails.pi) {
      ...
      this.ctx.fillText(
        "0 & 2π",
        circleCentreX + radius + radianUnitLen * 4,
        circleCentreY
      );

      this.ctx.textBaseline = "bottom";
      let [lineEndX, lineEndY] = this.circle.circleEndOffsetCoords(
        degToRad(30),
        radianUnitLen * 4
      );
      this.ctx.fillText("π/6", lineEndX, lineEndY);
      [lineEndX, lineEndY] = this.circle.circleEndOffsetCoords(
        degToRad(45),
        radianUnitLen * 4
      );
      ...
      this.ctx.fillText(
        "π/2",
        circleCentreX,
        circleCentreY - radius - radianUnitLen * 4
      );
  ...
  onMouseMove(e) {
    ...
    if (
      Math.sqrt(
        (mouseX - circleCentreX) * (mouseX - circleCentreX) +
          (mouseY - circleCentreY) * (mouseY - circleCentreY)
      ) < radius
    ) {
      this.resetCanvas();
